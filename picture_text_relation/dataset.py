...
class MyDataPoint: # 基础数据点类
    def __init__(self):
        self.feat: Optional[torch.Tensor] = None # 这行定义了一个名为feat的属性
       # 它的类型被注解为Optional[torch.Tensor]，意味着它可以是一个torch.Tensor对象或None
...
class MyPair(MyDataPoint):
    def __init__(self, sentence, image,image_explan, label=-1):
        super().__init__()
        self.sentence: MySentence = sentence
        self.image: MyImage = image
        self.image_explan : MySentence = image_explan
        self.label = label
